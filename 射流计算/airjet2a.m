...
for re=1:1
    ...
    if num == 1
        % 第一次计算：使用 0 作为搜索起点
         alpha_target = ast-alphaj(numj,1)*i;
    else
...
%%%%% Dispersion plots %%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% MATLAB: imagesc 叠加 contour 完整代码示例

    % figure();
    % pcolor(alphaArray, -alphaj, Sesan');
    % shading interp; % 平滑色彩过渡，去掉黑色网格线
    % colorbar;
    % % colormap(hot);
    % xlabel('$alpha, \hspace{0.2cm} \tilde{k}_r$','Interpreter','Latex')
    % ylabel('$alpha, \hspace{0.2cm} \tilde{k}_i$','Interpreter','Latex')
    % hold on;
    [C, h] =contour(alphaArray, -alphaj, Sesan',10);
