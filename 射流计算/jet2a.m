...
Rel_matrix = [100; 100];	% Reynolds number using liquid viscosity, Ujet and radius of the jet (There are 2 values which are 
                   
Wel_matrix = [3; 3];    % Weber number using liquid density, Ujet, radius of the jet and the surface tension coefficient, gamma
...
Factor1=kkk/(Ugstar-1);
y1 = (rL+1)/2;
y1a= (1+1)/2;

Ul = (Ugstar-1)*tanh(Factor1*(y1-1))+Ugstar;
Ul = Ul/Ujet;

Ul_int = (Ugstar-1)*tanh(Factor1*(y1a-1))+Ugstar;
Ul_int = Ul_int/Ujet;     % interface value

% First derivative of velocity
Ulp = (Ugstar - 1) * Factor1 * (1./cosh(Factor1 * (y1 - 1))).^2;
Ulp = Ulp/Ujet;

Ulp_int = (Ugstar - 1) * Factor1 * (1./cosh(Factor1 * (y1a - 1))).^2;
Ulp_int = Ulp_int/Ujet;   % interface value

% Second derivative of velocity
Ulpp = -2 * (Ugstar - 1) * Factor1^2 * (1./cosh(Factor1 * (y1 - 1))).^2 .* tanh(Factor1 * (y1 - 1));
Ulpp = Ulpp/Ujet;

Ulpp_int = -2 * (Ugstar - 1) * Factor1^2 * (1./cosh(Factor1 * (y1a - 1))).^2 .* tanh(Factor1 * (y1a - 1));
Ulpp_int = Ulpp_int/Ujet; % interface value

%% Gas

Factor2 = kkk/nnn/(qqq-Ugstar); 
y2 = (rG*(1-lr)+(1+lr))/2;
y2a= (1*(1-lr)+(1+lr))/2;
% Base velocity (The equation is non-dimensionalized)

Ug = (qqq-Ugstar)*tanh(Factor2*(y2-1))+Ugstar;
Ug = Ug/Ujet;

Ug_int = (qqq-Ugstar)*tanh(Factor2*(y2a-1))+Ugstar;
Ug_int = Ug_int/Ujet;     % interface value

% First derivative of velocity
Ugp = (qqq - Ugstar) * Factor2 .* (1./cosh(Factor2 * (y2 - 1))).^2;
Ugp = Ugp/Ujet;

Ugp_int = (qqq - Ugstar) * Factor2 .* (1./cosh(Factor2 * (y2a - 1))).^2;
Ugp_int = Ugp_int/Ujet;   % interface value

% Second derivative of velocity
Ugpp = -2 * (qqq - Ugstar) * Factor2^2 .* (1./cosh(Factor2 * (y2 - 1))).^2 .* tanh(Factor2 * (y2 - 1));
Ugpp = Ugpp/Ujet;

Ugpp_int =  -2 * (qqq - Ugstar) * Factor2^2 .* (1./cosh(Factor2 * (y2a - 1))).^2 .* tanh(Factor2 * (y2a - 1));
...
for re=1:1
    ...
    alphaj = (ajst:mj:ajend)';
    Sesan = zeros(length(alphaArray),length(alphaj));
    
    for numj=1:length(alphaj)
    for num=1:length(alphaArray)
      alpha = alphaArray(num,1)-alphaj(numj,1)*i;    % This is the non-dimensional frequency
    ...
    % C0 matrix
    C0_liq_11 = 2*D1_liq + (2./(rL+1)).*D0_liq;  % radial velocity part       v'+1/r*v
    ...
    % C0 matrix
    C0_liq_21 = (4/Rel).*D2_liq + (4./(Rel*(rL+1))).*D1_liq - ((4./(Rel*((rL+1).^2))) + 1./Rel*alpha^2 + 1i*alpha*Ul).*D0_liq;  
    % radial velocity part      1/Re*v''+1/Re/r*v'-(1/Re/r^2+1/Re*α^2+i*alpha*U)*v
    C0_liq_23 = zeros(Nl-1,Nl+1);                       % axial velocity part
    C0_liq_24 = -2*D1_liq;                              % pressure part   -p'
    ...
    C0_liq_43 = (4/Rel)*D2_liq + (4./(Rel*(rL+1))).*D1_liq - 1/Rel*alpha^2*D0_liq - 1i*alpha*Ul.*D0_liq; 
    ...
    % C0 matrix
    C0_gas_21 = (Vr/Dr)*(4/(Rel*(1-lr)^2))*D2_gas + (Vr/Dr)*(4./(Rel*(1-lr)*(rG*(1-lr)+lr+1))).*D1_gas -...
        ( (Vr/Dr)*(4./(Rel*((rG*(1-lr)+lr+1).^2))) + (Vr/Dr)/Rel*alpha^2 + 1i*alpha*Ug).*D0_gas;	
    % radial velocity part  1/Re*v''+1/Re/r*v'-(1/Re/r^2+1/Re*α^2+i*alpha*U)*v
    C0_gas_23 = zeros(Ng-1,Ng+1);        % axial velocity part
    C0_gas_24 = -2/((1-lr))/Dr*D1_gas;      % pressure part       -p'
    ...
    C0_gas_43 = (Vr/Dr)*(4/(Rel*(1-lr)^2))*D2_gas + (Vr/Dr)/Rel*(4./((1-lr)*(rG*(1-lr)+lr+1))).*D1_gas -...
    ...
    tempC0_liq = [-4/Rel*D1_liqInterface 1/Wel*(alpha^2-1)*(1/(Ulp_int-Ugp_int))*D0_liqInterface D0_liqInterface(:,1:end-1)];
    tempC1_liq = [zeros(1,Nl+1) zeros(1,Nl+1) zeros(1,Nl)];
   
    

    tempC0_gas = [4*Vr/(Rel*(1-lr))*D1_gasInterface 1/Wel*(-alpha^2+1)*(1/(Ulp_int-Ugp_int))*D0_gasInterface -D0_gasInterface(:,1:end-1)];
    ...
    tempC0_liq = [-(alpha^2*Uint)*D0_liqInterface 1i*alpha*Uint*2*D1_liqInterface zeros(1,Nl)];
    tempC1_liq = [alpha*D0_liqInterface -1i*2*D1_liqInterface zeros(1,Nl)];
    % 
    tempC0_gas = [Vr*alpha^2*Uint*D0_gasInterface -1i*alpha*Vr*Uint*2/(1-lr)*D1_gasInterface zeros(1,Ng)]; 
    tempC1_gas = [-alpha*Vr*1i*D0_gasInterface 1i*Vr*2/(1-lr)*D1_gasInterface zeros(1,Ng)];

    % tempC0_liq = [1i*alpha*D0_liqInterface 2*D1_liqInterface zeros(1,Nl)];
    % tempC1_liq = [zeros(1,Nl+1) zeros(1,Nl+1) zeros(1,Nl)];
    % 
    % tempC0_gas = [-Vr*1i*alpha*D0_gasInterface -Vr*2/(1-lr)*D1_gasInterface zeros(1,Ng)]; 
    % tempC1_gas = [zeros(1,Ng+1) zeros(1,Ng+1) zeros(1,Ng)];
    ...
    tempC0_liq = [(Ulp_int-Ugp_int)*D0_liqInterface alpha*Uint*1i*D0_liqInterface zeros(1,Nl)];
    tempC1_liq = [zeros(1,Nl+1) -1i*D0_liqInterface zeros(1,Nl)];

    tempC0_gas = [zeros(1,Ng+1) -alpha*Uint*1i*D0_gasInterface zeros(1,Ng)];
    ...
    Sesan(:,numj) = GrowthRate;
  
    end
    end
[X, Y] = meshgrid(alphaArray, -aend/ajend*alphaj);
Z = zeros(size(X)); % ParaView 默认处理三维，二维平面将 Z 设为 0
V = Sesan';         % 你的标量场数据

[ny, nx] = size(V);

% 2. 打开文件
fid = fopen('Sesan_Data.vtk', 'w');

% 3. 写入 VTK 文件头
fprintf(fid, '# vtk DataFile Version 3.0\n');
fprintf(fid, 'Sesan Data Export from MATLAB\n');
fprintf(fid, 'ASCII\n');
...
% 5. 写入标量数据 (Sesan)
fprintf(fid, '\nPOINT_DATA %d\n', nx * ny);
fprintf(fid, 'SCALARS Sesan float\n');
